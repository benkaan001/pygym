...
"""
    Step 1 -> sort the populations dict by population value in descending order
    Step 2 -> extract first 3 values
    Step 3 -> extract the state names from the resulting tuple list in Step 2
    Step 4 -> iterate through capitals list to extract the matching capitals
...
sorted_state_list = [(state,pop) for state,pop in sorted(populations.items(), key= lambda x: x[1], reverse = True)]
print(reprlib.repr(sorted_state_list)) 
# [('ca', 38332521), ('tx', 26448193), ('ny', 19651127), ('fl', 19552860), ('il', 12882135), ('pa', 12773801), ...]

highest_pop_state_list = sorted_state_list[:3]
print(highest_pop_state_list) 
# [('ca', 38332521), ('tx', 26448193), ('ny', 19651127)]

highest_pop_state_names = [x[0] for x in highest_pop_state_list]
print(highest_pop_state_names) # ['ca', 'tx', 'ny']

highest_pop_state_capitals = [capital for state,capital in capitals.items() if state in highest_pop_state_names]
print(highest_pop_state_capitals) # ['Sacramento', 'Albany', 'Austin']
