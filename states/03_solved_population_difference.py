...
"""
    ...
    STEP 1 -> Iterate through admissions dict to create state tuples in descending order
    STEP 2 -> Extract the first and last items
    STEP 3 -> Iterate through populations to extract population values for oldest and
              youngest states 
    STEP 4 -> Print the absolute value of population difference 
            

"""

 
state_tuples = [(key,value) for key,value in sorted(admissions.items(), key= lambda x: x[1])]
print(state_tuples)

oldest, youngest = state_tuples[0], state_tuples[len(state_tuples)-1]
print(oldest) # ('de', 1787)
print(youngest) # ('hi', 1959)

oldest_state_pop, youngest_state_pop = \
[v for k,v in populations.items() if k == oldest[0] or k == youngest[0]]
print(oldest_state_pop) # 925749
print(youngest_state_pop) # 1404054

print(abs(oldest_state_pop - youngest_state_pop)) # 478305